...
public class MethodInfo implements AbstractMethodInfo {
  ...
  public MethodInfo(String name, String returnType, boolean isAbstract, boolean isNative,
      boolean isSynchronized, boolean isStatic, boolean isFinal, String deprecated, String scope,
  ...
  public boolean isConsistent(MethodInfo mInfo) {
    ...
    if (!mReturn.equals(mInfo.mReturn)) {
      ...
      Errors.error(Errors.CHANGED_TYPE, mInfo.position(), "Method " + mInfo.qualifiedName()
          + " has changed return type from " + mReturn + " to " + mInfo.mReturn);
    }
      ...
      if (!mInfo.mExceptions.contains(exec)) {
        ...
        if (!name().equals("finalize") || (mParameters.size() > 0)) {
          Errors.error(Errors.CHANGED_THROWS, mInfo.position(), "Method " + mInfo.qualifiedName()
              + " no longer throws exception " + exec);
          consistent = false;
        }
      }
    }
